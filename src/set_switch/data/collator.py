"""Batch collation for rendered SetSwitch examples."""

from __future__ import annotations

from collections.abc import Sequence
from dataclasses import dataclass
from typing import Any

import torch

from set_switch.constants import DOC_CAUSAL, IGNORE_INDEX, ROLE_PAD
from set_switch.modeling.attention_mask import build_setswitch_attention_mask


@dataclass
class SetSwitchCollator:
    tokenizer: Any
    attention_mode: str = DOC_CAUSAL
    mask_dtype: torch.dtype = torch.float32
    pad_to_multiple_of: int | None = None

    def __call__(self, features: Sequence[dict[str, Any]]) -> dict[str, Any]:
        if not features:
            raise ValueError("Cannot collate an empty batch")

        pad_token_id = self.tokenizer.pad_token_id
        if pad_token_id is None:
            raise ValueError("Tokenizer must have a pad_token_id")

        max_len = max(len(feature["input_ids"]) for feature in features)
        if self.pad_to_multiple_of:
            multiple = int(self.pad_to_multiple_of)
            if max_len % multiple:
                max_len = ((max_len // multiple) + 1) * multiple

        batch: dict[str, list[Any]] = {
            "input_ids": [],
            "labels": [],
            "role_ids": [],
            "item_ids": [],
            "read_slot_ids": [],
            "gather_slot_ids": [],
            "position_ids": [],
            "pad_mask": [],
            "answer_start": [],
            "example_id": [],
        }

        for feature in features:
            length = len(feature["input_ids"])
            pad_len = max_len - length
            batch["input_ids"].append(feature["input_ids"] + [pad_token_id] * pad_len)
            batch["labels"].append(feature["labels"] + [IGNORE_INDEX] * pad_len)
            batch["role_ids"].append(feature["role_ids"] + [ROLE_PAD] * pad_len)
            batch["item_ids"].append(feature["item_ids"] + [-1] * pad_len)
            batch["read_slot_ids"].append(feature["read_slot_ids"] + [-1] * pad_len)
            batch["gather_slot_ids"].append(feature["gather_slot_ids"] + [-1] * pad_len)
            batch["position_ids"].append(feature["position_ids"] + [0] * pad_len)
            batch["pad_mask"].append([True] * length + [False] * pad_len)
            batch["answer_start"].append(feature["answer_start"])
            batch["example_id"].append(feature.get("example_id", ""))

        role_ids = torch.tensor(batch["role_ids"], dtype=torch.long)
        item_ids = torch.tensor(batch["item_ids"], dtype=torch.long)
        read_slot_ids = torch.tensor(batch["read_slot_ids"], dtype=torch.long)
        gather_slot_ids = torch.tensor(batch["gather_slot_ids"], dtype=torch.long)
        pad_mask = torch.tensor(batch["pad_mask"], dtype=torch.bool)

        attention_mask = build_setswitch_attention_mask(
            role_ids=role_ids,
            item_ids=item_ids,
            read_slot_ids=read_slot_ids,
            gather_slot_ids=gather_slot_ids,
            attention_mode=self.attention_mode,
            pad_mask=pad_mask,
            dtype=self.mask_dtype,
        )

        return {
            "input_ids": torch.tensor(batch["input_ids"], dtype=torch.long),
            "labels": torch.tensor(batch["labels"], dtype=torch.long),
            "attention_mask": attention_mask,
            "position_ids": torch.tensor(batch["position_ids"], dtype=torch.long),
            "role_ids": role_ids,
            "item_ids": item_ids,
            "read_slot_ids": read_slot_ids,
            "gather_slot_ids": gather_slot_ids,
            "pad_mask": pad_mask,
            "answer_start": torch.tensor(batch["answer_start"], dtype=torch.long),
            "example_id": batch["example_id"],
        }
